...
const stageHandlers: Record<
  PipelineStage,
  (job: Job<RenderJobData>, video: Record<string, unknown>) => Promise<void>
> = {
...
async function processJob(job: Job<RenderJobData>): Promise<void> {
  ...
  for (let i = 0; i < stages.length; i++) {
    ...
    try {
      ...
      await stageHandlers[stage](job, video);
...
async function handleScriptGeneration(
  ...
  const generatedScript =
    (video.script as string) || `Generated script for video: ${video.title}`;
